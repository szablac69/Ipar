...
public class LdIconNO extends LdIconBase{

	public static final int cmd = 0x02;
	public static final int textureXpos = 0;
	public static final int textureYpos = 0;
	
	public static final int maxItem = 64;
	
	public LdIconNO(int x, int y) {
		super(textureXpos, textureYpos, (byte)cmd, x, y);
		this.address = new Address24bit();
	}
	public LdIconNO(int value, int x, int y) {
	...
	public List<Instruction> getInstructionList(TileEntityPLC PLC, int posX,int posY) {
		...
		lstInst.add(new net.ipar.mod.utilsPLC.Instruction(EnuStlCmd.ST, this.address));
		return lstInst;
	}
